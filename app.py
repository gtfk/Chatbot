# Versión 7.9 (Final) - Registro corregido (Hasher.hash) y logos OK
...
if not st.session_state["authentication_status"]:
    with st.sidebar:
        ...
        with tab_reg:
             # --- CORRECCIÓN AQUÍ: st.form en lugar de register_user ---
             with st.form("registro"):
                ...
                if st.form_submit_button("Registrarse"):
                    if new_name and new_email and len(new_pass) >= 6:
                        try:
                            # Usamos la forma moderna y correcta del hasher
                            hasher = stauth.Hasher()
                            hashed_pass = hasher.hash(new_pass)
                            ...
                            st.success("Cuenta creada. Ve a la pestaña 'Login' e inicia sesión.")
                        except Exception as e:
                            st.error(f"Error al crear cuenta: {e}")
                    else:
                        st.error("Por favor completa todos los campos (contraseña mín. 6 caracteres).")
             # --- FIN CORRECCIÓN ---
